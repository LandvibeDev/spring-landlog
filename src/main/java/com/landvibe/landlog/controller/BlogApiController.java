...
public class BlogApiController {
    ...
    @GetMapping()
    public List<Blog> list(@RequestParam long creatorId) throws MemberException {
        List<Blog> blogs = blogService.findBlogsByCreatorId(creatorId);
        return blogs;
    }

    @PostMapping()
    public Blog create(@RequestParam long creatorId, @RequestBody Blog blog) throws MemberException {
        Blog write = blogService.write(blog);
        return write;
    }

    @GetMapping(value = "/{blogId}")
    public Blog update(@PathVariable long blogId, @RequestParam long creatorId) throws BlogException {
